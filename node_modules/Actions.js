module.exports = function(ScreenBlocks) {
     ...
     */
    this.unreadCount = function(callback1, callback2) {
    ...
    */
    ...
    this._date = function(date) {
        ...
        return f(d.getHours()) + ':' + f(d.getMinutes()) + '  ' + f(d.getDate()) + '.' + f(d.getMonth() + 1) + '.' + d.getFullYear();
    };
    ...
    // =================================
    // проверка пользователей online
    this.online = function() {
        vk.request('friends.getOnline', {
            version: 5.28
        }, function(data) {
            for (var j = 0; j < uidFriends.length; ++j) {
                uidFriends[j].online = false;
            }

            if (data && data.response) {
                for (var i = 0; i < data.response.length; ++i) {
                    if (uidFriends[data.response[i]] !== undefined) {
                        uidFriends[data.response[i]].online = true;
                    }
                }
            }
        });
    ...
    var _friend = function(id) {
        ...
        if (uidFriends[id] !== undefined) {
            return (uidFriends[id].online ? '⍟ ' : '') + uidFriends[id].name;
        }

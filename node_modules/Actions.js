module.exports = function(ScreenBlocks) {
     ...
     */
    this.getHistory = function(id, been_err) {
        ...
        vk.request('messages.getHistory', {
            ...
            if (data && data.response && data.response.items) {
                ...
                for (var i = r.length - 1; i >= 0; i--) {
                    mid.push(r[i].id);
                    content = content + '{bold}' + _friend(r[i].from_id) + '{/bold} ' + _date(r[i].date) + "\n" + r[i].body + "\n\n";
                }
                ...
                // если ошибка, то отправляем сообщение снова,
                // но с ключем который остановит цикл в случае ноовой ошибки
                if(!been_err){
                    setTimeout(function() {
                        getHistory(id, true);
                    }, 500);
    ...
    this.send = function(Text, id) {

        if(Text === '' || Text === undefined || id === undefined || id === 0|| id === '0' ){
            return false;
        }
